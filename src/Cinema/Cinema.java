...
public class Cinema implements Serializable{
	...
	public String toString() {
		...
		return "Кінотеатр " + name + " працює з " + open + " до " + close + "\nПереглядати фільми можна в "
				+ halls.size() + " залах: " + hallsNames;
	}
	...
	void addSeance(String hallName, Days day, Seance s) {
		...
		if (s.getEndTime().compareTo(close) == 1)
			throw new NotCorrectTimeException("Час сеансу не співпадає з часом роботи кінотеатру!");
	...
	void addMovie(Movie movie, String hallName, Days day, Time... times) {
		...
		Arrays.asList(times).stream().forEach(t -> {
			try {
				...
				System.out.print("Сеанс в "+t+" не додано. ");
